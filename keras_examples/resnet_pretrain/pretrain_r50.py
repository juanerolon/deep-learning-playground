...
#*************************** PRE_TRAINED_MODEL ********************************************
#Load pre-trained features otained by a pre-trained Resnet50 convolutional neural network
...
#Quick model build-up: join the pre-trained Resnet50 feature layer wit the output layer
...
print("Current directory for checkpointer object:")
print(current_dir)
print("\n")
...
checkpointer = ModelCheckpoint(filepath='saved_models/optimized_adamax.ResNet50.hdf5',
...
if False:
    h = ResNet_model.fit(train_ResNet50, train_targets,
          validation_data=(valid_ResNet50, valid_targets),
          epochs=epochs, batch_size=batch_size, callbacks=[checkpointer], verbose=1)

# Set to True to fit the model to the augmented datasets
if True:
    h = ResNet_model.fit_generator(datagen_train.flow(train_ResNet50, train_targets, batch_size=batch_size),
...
ResNet_model.load_weights('saved_models/optimized_adamax.ResNet50.hdf5')


# get index of predicted dog breed for each image in test set
ResNet_predictions = [np.argmax(ResNet_model.predict(np.expand_dims(feature, axis=0))) for feature in test_Xception]
